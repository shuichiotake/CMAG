...
class Eval(object):
    def __init__(self, knd, e, sent_num_dict, noised_mat):
        self.knd = knd
        self.e = e
        self.sent_num_dict = sent_num_dict
    ...
    def batcher(self, params, batch): 
        sent_num_dict = self.sent_num_dict
        noised_mat = self.noised_mat
        sentences = [' '.join(s) for s in batch]
        try:
            nums = [int(sent_num_dict[sent]) for sent in sentences]
        except KeyError:
            print(sentences)
